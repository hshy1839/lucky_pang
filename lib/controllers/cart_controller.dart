...
class CartController {
  ...
  Future<List<Map<String, dynamic>>> fetchCartData(String userId, String token) async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.219.107:7778/api/cart/$userId'),
  ...
  static const String apiUrl = 'http://192.168.219.107:7778/api/cart'; // 서버 주소
  ...
  static Future<void> deleteCartItem(String cartId) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('http://192.168.219.107:7778/api/cart/delete/$cartId'),
        headers: {
          'Authorization': 'Bearer $token', // JWT 토큰을 헤더에 추가
        },
      );
